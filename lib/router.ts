import { htmlStringToDocument, replaceBody } from './dom/body';
import { mergeHead } from './dom/head';
import { runScripts } from './dom/scripts';
import { handleMouseClick, handlePopState } from './handlers';
import { updateHist } from './history';
...
export class Router {
  ...
  constructor(opts: FlamethrowerOptions) {
    ...
    window.addEventListener('scroll', updateHist);
  ...
  public go(path: string): Promise<boolean> {
    ...
    return this.reconstructDOM({ type: TransitionType.Link, next, prev, scroll: scrollId });
  }
  ...
  private addLinkPrefetch(href: string): void {
    ...
    linkEl.onload = () => this.log('🌩️ prefetched', href);
    // eslint-disable-next-line quotes
    linkEl.onerror = (err) => this.log("🤕 can't prefetch", href, err);
  ...
  private async reconstructDOMUnsafe(routeChangeData: RouteChangeData): Promise<boolean> {
    ...
    if (routeChangeData.type === TransitionType.Scroll) {
      scrollTo(routeChangeData.type, routeChangeData.scroll);
      return false;
    }

    if (routeChangeData.next === routeChangeData.prev) return false;

    const { type, next, scroll: scrollId } = routeChangeData;
...
export function addToPushState(url: string): void {
  ...
  if (window.history.state?.__flame?.url !== url) {
...
export function scrollTo(type: TransitionType, pos: string | number | null): void {
  const behavior = type === TransitionType.Scroll ? 'smooth' : 'auto';
  if (typeof pos === 'number') return window.scrollTo({ top: pos, behavior });

  const el = pos ? document.querySelector(pos) : null;
  if (!el) return window.scrollTo({ top: 0 });

  // When we click a link, it doesn't look good to smooth scroll
